
using TMPro;
using UnityEngine;
using UnityEngine.UI;
using DG.Tweening;
using UnityEngine.EventSystems;


public class ItemMenu : MonoBehaviour
{
    [SerializeField] private GameObject menu;
    private Animator leoraAnimator;
    public Sprite idleSprite;
    private SpriteRenderer sr;
    //Reference Leora if we want her sprite to change? should I just do an overlay? I might just do that


    [SerializeField] private Sprite[] items;
    //Sprites in the array: 0 for attack, 1 for MP, 2 for HP

    [SerializeField] private Image itemImage;
    [SerializeField] private TMP_Text itemText;
    [SerializeField] private TMP_Text itemDescription;
    [SerializeField] private EquipmentManager equipmentManager;

    //For Movement
    [SerializeField] private Transform infoHolder;
    [SerializeField] private Transform[] locations;

    [SerializeField] private GameObject contButton;
    [SerializeField] private GameObject key;
    [SerializeField] private GameObject goToPauseUI;

    [SerializeField] private CanvasGroup itemHolder;

    public void CloseMenu()
    {
        if (leoraAnimator.gameObject.name == "Player")
        {
            sr = GameObject.FindGameObjectWithTag("Player").GetComponent<SpriteRenderer>();
            sr.sprite = idleSprite;
        }
        
        /*else if (leoraAnimator.gameObject.name == "CombatPlayer")
        {
            sr = GameObject.FindGameObjectWithTag("Player").GetComponentInChildren<SpriteRenderer>();
        }*/
        

        EventSystem.current.SetSelectedGameObject(null);
        OpenPauseMenu.pauseOpened = false;
        itemImage.gameObject.SetActive(false);
        leoraAnimator.enabled = true;
        infoHolder.DOMove(locations[1].position, 0.5f).SetUpdate(true).OnComplete(() =>
        //itemHolder.DOFade(0, 1f).SetUpdate(true).OnComplete(() =>
        {
            infoHolder.position = locations[2].position;
            //menu.SetActive(false);
            OpenPauseMenu.GLOBALcanOpenPause = true;
            if (OpenPauseMenu.canOpenPause)
            {
                Time.timeScale = 1;
            }
        });
    }

    public void ChangeTextAndSprite(string dropName)
    {
        //Commented out the lines about assigning the image, will add back later
        //Debug.Log(dropName);
        if(goToPauseUI != null)
        {
            goToPauseUI.SetActive(false);
        }
        switch (dropName)
        {
            case "BloodAmulet":
                itemText.text = "Amulet of the Body";
                itemImage.sprite = items[0];
                itemDescription.text = "<color=#92dae8>Allows Leora to use Blood Magic.</color> The gemstone of the amulet is a deep burgundy, so deep it almost looks like blackberry jam. " +
                    "When rubbed, it coats your fingers in an odd red substance, not quite blood but not quite... normal.";
                break;
            case "MindAmulet":
                itemText.text = "Amulet of the Mind";
                itemImage.sprite = items[1];
                itemDescription.text = "<color=#92dae8>Allows Leora to use Psychic Magic.</color> Unstable is the only way to describe this amulet, the gemstone cracking from the inside out. " +
                    "The longer it is worn, the more cracks appear. It is only a matter of time before it crumbles, yet it is still passed from wearer to wearer.";
                break;
            case "DarkAmulet":
                itemText.text = "Amulet of the Heart";
                itemImage.sprite = items[2];
                itemDescription.text = "<color=#92dae8>Allows Leora to use Dark magic.</color> This amulet is rough around the edges, carved into the shape of a heart with kind yet inexperienced hands. " +
                    "This once pure gemstone has eroded with time, the clear quartz now a dull, muted black.";
                break;
            case "AlanAmulet":
                itemText.text = "Amulet of Tenacity";
                itemImage.sprite = items[3];
                itemDescription.text = "<color=#92dae8>Increases Leora's attack based on how many criminals are condemned.</color> Even in the deepest darkness, this amulet shines– it's fiery orange subtle yet persistent.";
                break;
            case "KisaAmulet":
                itemText.text = "Amulet of Performance";
                itemImage.sprite = items[4];
                itemDescription.text = "<color=#92dae8>Health and Mana pickups restore double their original value.</color> The amulet emits a gentle hum, even if not worn. It is a calming tune, one that could turn the most horrid monster docile.";
                break;
            case "SophieAmulet":
                itemText.text = "Amulet of Thunderstorm";
                itemImage.sprite = items[5];
                itemDescription.text = "<color=#92dae8>Allows Leora to critical hit for double damage but reduces base attack while equipped.</color> The gem of this amulet crackles and shimmers, almost as if there was a storm rumbling within a glassy enclosure.";
                break;
            case "AdvATKRing":
                itemText.text = "Ring of Advanced Power";
                itemImage.sprite = items[6];
                itemDescription.text = "<color=#92dae8>Greatly increases Leora's attack.</color> The gemstone sparkles a vibrant vermillion, the metal polished and begging to be worn.";
                break;
            case "AdvHPRing":
                itemText.text = "Ring of Advanced Vitality";
                itemImage.sprite = items[7];
                itemDescription.text = "<color=#92dae8>Increases Leora's Health by 50 points.</color> The gemstone glitters like leaves dancing in the wind, the metal polished and begging to be worn.";
                break;
            case "AdvMPRing":
                itemText.text = "Ring of Advanced Mana";
                itemImage.sprite = items[8];
                itemDescription.text = "<color=#92dae8>Increases Leora's Mana by 10 points.</color> The gemstone shimmers like the sun illuminating a sea, the metal polished and begging to be worn.";
                break;
            case "ATKRing":
                itemText.text = "Ring of Power";
                itemImage.sprite = items[9];
                itemDescription.text = "<color=#92dae8>Slightly increases Leora's attack.</color> The gemstone gives a subtle glimpse of crimson, but it is weakening. The metal was once high-quality, but has begun withering with time.";
                break;
            case "MPRing":
                itemText.text = "Ring of Mana";
                itemImage.sprite = items[10];
                itemDescription.text = "<color=#92dae8>Increases Leora's Mana by 5 points.</color> The gemstone gives a subtle glimpse of ocean blue, but it is weakening. The metal was once high-quality, but has begun withering with time.";
                break;
            case "HPRing":
                itemText.text = "Ring of Vitality";
                itemImage.sprite = items[11];
                itemDescription.text = "<color=#92dae8>Increases Leora's Health by 25 points.</color> The gemstone gives a subtle glimpse of emerald green, but it is aging. The metal was high-quality, but has begun withering with time.";
                break;
            case "ATKMPRing":
                itemText.text = "Ring of Might and Magic";
                itemImage.sprite = items[12];
                itemDescription.text = "<color=#92dae8>Increases Leora's Mana by 5 points and slightly increase Leora's attack.</color> Crimson and teal fuse into one in this ring's gemstone, symbolizing the unity of might and magic.";
                break;
            case "HPMPRing":
                itemText.text = "Ring of Vigor and Magic";
                itemImage.sprite = items[13];
                itemDescription.text = "<color=#92dae8>Increases Leora's HP by 25 points and her Mana by 5 points</color> Forest green and teal fuse into one in this ring's gemstone, symbolizing the unity of vigor and magic.";
                break;
            case "ATKHPRing":
                itemText.text = "Ring of Might and Vigor";
                itemImage.sprite = items[14];
                itemDescription.text = "<color=#92dae8>Increases Leora's HP by 25 points and slightly increase Leora's attack.</color> Crimson and forest green fuse into one in this ring's gemstone, symbolizing the unity of might and vigor.";
                break;
            case "veritaatkbonus":
                itemText.text = "Attack Bonus";
                itemImage.sprite = items[16];
                itemDescription.text = "Verita has blessed you with their strength. <color=#92dae8>Leora gains a permanent bonus to her attack.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "veritampbonus":
                itemText.text = "Magic Bonus";
                itemImage.sprite = items[17];
                itemDescription.text = "Verita has blessed you with their wisdom. <color=#92dae8>Leora gains a permanent bonus to her magic attack.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "veritahpbonus":
                itemText.text = "Health Bonus";
                itemImage.sprite = items[18];
                itemDescription.text = "Verita has blessed you with their courage. <color=#92dae8>Leora gains a permanent +25 to her health.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "orderatkbonus":
                itemText.text = "Attack Bonus";
                itemImage.sprite = items[19];
                itemDescription.text = "The Order has granted you a sharper weapon. <color=#92dae8>Leora gains a permanent bonus to her attack.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "ordermpbonus":
                itemText.text = "Magic Bonus";
                itemImage.sprite = items[20];
                itemDescription.text = "The Order has granted you an insightful tome. <color=#92dae8>Leora gains a permanent bonus to her magic attack.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "orderhpbonus":
                itemText.text = "Health Bonus";
                itemImage.sprite = items[21];
                itemDescription.text = "The Order has bestowed you with new armor. <color=#92dae8>Leora gains a permanent +25 to her health.</color>";
                goToPauseUI.SetActive(false);
                break;
            case "Key":
                itemText.text = "Key";
                itemImage.sprite = items[22];
                itemDescription.text = "This <i>must</i> unlock something important...";
                goToPauseUI.SetActive(false);
                key.SetActive(true);
                break;
            default:
                Debug.Log("I couldn't find the item equipped. Sorry pookie");
                break;
        }
    }

    public void openItemMenu(string itemName)
    {
        audioManager.Instance.playSFX(44);
        OpenPauseMenu.pauseOpened = true;
        Time.timeScale = 0;
        OpenPauseMenu.GLOBALcanOpenPause = false;

        ChangeTextAndSprite(itemName);

        // only do this if the itemName doesn't end with bonus
        if (!itemName.EndsWith("bonus"))
        {
            equipmentManager.GainedEquipment(itemName);
        }

        infoHolder.DOMove(locations[0].position, 0.5f).SetUpdate(true).OnComplete(() =>
        {
            EventSystem.current.SetSelectedGameObject(null);
            EventSystem.current.SetSelectedGameObject(contButton);
        });
        //itemHolder.DOFade(1, 1f).SetUpdate(true);
        itemImage.gameObject.SetActive(true);
        leoraAnimator = GameObject.FindGameObjectWithTag("Player").GetComponent<Animator>();
        leoraAnimator.enabled = false;
        leoraAnimator.GetComponent<SpriteRenderer>().sprite = items[15];
    }

    public void Awake()
    {
        equipmentManager = FindObjectOfType<EquipmentManager>();
        if(key != null && MansionDoorManager.hasKey == true)
        {
            key.SetActive(true);
        }
    }
}
